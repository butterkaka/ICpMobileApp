...
import { App, IonicPage, NavController, NavParams, AlertController, Keyboard } from 'ionic-angular';
...
export class ConnectorParameterSetupPage {
  ...
  displayUnit = 4;  //blir tokfel, alltid PSI
  graphHeader;
  graphValue;
  minInput = 0;
  MaxInput = 0;
  ...
  data = {
    ...
    datasets: [
      ...
      },
      {
        label: "",
        fill: false,
        backgroundColor: "rgba(223,0,36,0.4)",  //red
      ...
      },
      {
        label: "",
        fill: false,
        backgroundColor: "rgba(253,172,16,0.4)",  //yellow
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public pcmchanneldataservice: PCMChannelDataService, private cd: ChangeDetectorRef,
    ...
    if (this.connector == Constants.values.PT) {
      ...
      this.analogInputParameterItems = pcmchanneldataservice.analogInputParameterItems;
  ...
  */
  ionViewDidEnter() {
    ...
    const done = new Promise((resolve, reject) =>{
      ...
      this.setParameterChannel();
      this.functionInputChange();
      resolve(this.readParameterValuesFromPCM());
    });
 ...
 */
  ...
  */
  async readParameterValuesFromPCM() {
    ...
    this.utilsService.write(this.deviceObject.deviceId, this.deviceObject.serviceUUID, this.deviceObject.characteristicId, byteArray1.buffer,0);
 ...
 */
  ...
  startNotify(deviceId, serviceUUID, characteristic) {
    ...
    this.ble.startNotification(deviceId, serviceUUID, characteristic).subscribe(buffer => {
      ...
      this.atmQuestionObjectList.push(responseObject);
      this.countAtmQList++;

      // for(let element of this.atmQuestionObjectList){
      //   if(responseObject.channel == element.channel && responseObject.subChannel == element.subChannel){
      //     exists = true;
      //     break;
      //   }
      //   else{
  ...
  */
  async writeConnecterSetupPinParameters() {
          ...
          if (connectorParamterSetupValuesList != null) {
            ...
            //connectorParamterSetupValuesList.forEach(element => {
              ...
              if (flag == 1) {
                ...
                byteArray = new Uint8Array([element.wType, 0, element.channel, element.subchannel, val1, val2, val3, val4]);
                await this.utilsService.write(this.deviceObject.deviceId, this.deviceObject.serviceUUID, this.deviceObject.characteristicId, byteArray.buffer);
              }
...
*/
  ...
  navigateToSettings() {
    ...
    this.app.getRootNav().push("SettingsPage", { deviceObject: this.deviceObject });
  }
