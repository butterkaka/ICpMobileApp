...
*/
      ...
      if (this.operation == Constants.values.initializeStartNotify) {
       ...
       //if(this.countAtmQList== 6)
        if (this.page == "ramp") {
          ...
          else if(this.operation == Constants.values.ReadConnectorParameterValues && this.countAtmQList >= (this.inputParameterCount)){
  ...
  */
  ...
  async readConnectorParameter(){
    this.operation = Constants.values.ReadConnectorParameterValues;
    this.atmQuestionObjectList = [];
    this.functionInputParameterList = [];
  ...
  populateConnectorParameterList(){
    ...
    let connectors = ["DO","DI","X4","X3","X2","X1","AI","PT"];
  ...
  */
  LoadValuesToParameterFileObject() {
    console.log("load started");
    this.atmQuestionObjectList.forEach(element => {
      this.parameterFileObject.forEach(parameterFileItem => {
  ...
  */
  ...
  CopyAllparameterListToMapID() {
    ...
    this.parameterFileObject.forEach(element => {
 ...
 */
  ...
  fileChooserFunction() {
    ...
    //this.pcmchanneldataservice.isResumedByPurpose = true;
    //this.pcmchanneldataservice.appResetFlag=false;
        ...
        if (fileFullPath.indexOf("content://") >= 0) {
          this.filePath.resolveNativePath(fileFullPath + fileName)
            .then(filePath => {
              ...
              console.log("resolve Native Path error:" + err);
              this.pcmchanneldataservice.presentAlert("Error when loading", err);
              this.pcmchanneldataservice.loaderGlobal.dismiss();
          });
        }
  ...
  */
  ...
  sendEmail() {
    ...
    let email = {
      ...
      subject: 'ICp Settings File: ' + this.datepipe.transform(new Date(), 'yyyy-MM-dd H:mm a'),
      isHtml: true
    };
    ...
    //   }
    // }).catch((ex:string)=>alert(ex + "email not available"));
 ...
 */
  ...
  readSetupParameters() {
    ...
    setupInputList.forEach(element => {
      ...
      this.utilsService.write(this.deviceObject.deviceId, this.deviceObject.serviceUUID, this.deviceObject.characteristicId, byteArray.buffer,5);
    });
