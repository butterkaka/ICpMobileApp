  ...
  */
  startNotify(deviceId, serviceUUID, characteristic) {
    this.ble.startNotification(deviceId, serviceUUID, characteristic).subscribe(buffer => {
      ...
      } else if (this.operation == Constants.values.numberChangeICp) {
...
*/
  ...
  numberChangePrompt() {
    if (this.pcmchanneldataservice.passwordFlag) {
      this.pcmchanneldataservice.alert = this.alertCtrl.create({
        title: Constants.messages.newIcpNumberTitle,
        subTitle: Constants.messages.newIcpNumberSubTitle,
        inputs: [
          {
            name: 'newNumber',
  ...
  newNumberWrite(newNumber) {
    this.operation = Constants.values.numberChangeICp;
    let channel = 40;
    let subChannel = 1;
    let wType = 3;
    let val1;
